...
from ..core.config import DEFAULT_CONFIG_DIR, CHAT_SESSIONS_DIR, ARTIFACTS_DIR, USAGE_DATA_DIR
...
class SettingsManager:
    ...
    def _get_default_settings(self) -> dict:
        ...
        return {
            "api_keys": {
                p.value: os.getenv(f"{p.name}_API_KEY", "") 
            ...
            "ui": {
                "selected_provider": "Anthropic",  # ← 기본 제공업체 설정
                "theme": "auto",
                "language": "ko"
            },
            "features": {
    ...
    def export_settings(self) -> Dict[str, Any]:
        ...
        if "api_keys" in export_data:
            export_data["api_keys"] = {
                k: "***" if v else "" 
                for k, v in export_data["api_keys"].items()
            }
        return export_data

    def import_settings(self, settings_data: Dict[str, Any], include_api_keys: bool = False):
        """설정 가져오기"""
