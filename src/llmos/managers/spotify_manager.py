...
class SpotifyManager:
    ...
    def __init__(self, settings_manager: SettingsManager):
        ...
        self._cache_dir = Path.home() / ".llmos" / "spotify_cache"
    ...
    def _load_spotify_settings(self):
        ...
        redirect_uri = settings.get("spotify_redirect_uri", "http://127.0.0.1:8888/callback")
        port_type = settings.get("spotify_port_type", "fixed")
        ...
        if client_id and client_secret:
            spotify_settings = SpotifySettings(
                ...
                port_type=port_type,
                cache_path=str(self._cache_dir / ".spotify_token_cache")
            )
    ...
    def save_spotify_settings(self, client_id: str, client_secret: str, 
                            ...
                            port_type: str = "fixed") -> bool:
        """Spotify 설정 저장"""
        try:
            # 설정 저장 (개별 항목에 대해 set 메서드 사용)
            self.settings_manager.set("spotify_client_id", client_id)
            self.settings_manager.set("spotify_client_secret", client_secret)
            self.settings_manager.set("spotify_redirect_uri", redirect_uri)
            self.settings_manager.set("spotify_port_type", port_type)
    ...
    def clear_cache(self):
        ...
        try:
            for cache_file in self._cache_dir.glob("*.json"):
                if cache_file.name != ".spotify_token_cache":  # 토큰 캐시는 유지
                    cache_file.unlink()
    ...
    def get_top_tracks(self, time_range: SpotifyTimeRange, limit: int = 100,
        ...
        cache_key = f"top_tracks_{time_range.value}_{limit}"
    ...
    def get_recent_frequent_tracks(self, days: int = 30, limit: int = 100,
        ...
        cache_key = f"recent_frequent_{days}_{limit}"
    ...
    def create_playlist(self, name: str, public: bool = False,
        ...
        if playlist:
            # 플레이리스트 목록 캐시 무효화
            self._get_cache_path("user_playlists").unlink(missing_ok=True)
    ...
    def remove_tracks_from_liked(self, track_ids: List[str]):
        ...
        # 좋아요 트랙 캐시 무효화
        self._get_cache_path("saved_tracks").unlink(missing_ok=True)
    ...
    def sort_playlist(self, playlist_id: str, sort_key: SpotifySortKey,
        ...
        if success:
            ...
            if new_playlist_name:
                self._get_cache_path("user_playlists").unlink(missing_ok=True)
            self._get_cache_path(f"playlist_tracks_{playlist_id}").unlink(missing_ok=True)
