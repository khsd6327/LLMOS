...
class ArtifactManager:
    ...
    def _load_index(self) -> Dict[str, dict]:
        ...
        if self.index_file.exists():
            try:
                ...
                logger.error(
                    f"Error decoding artifacts index: {self.index_file}. Creating new index."
                )
                return {}
        return {}
    ...
    def get(self, artifact_id: str) -> Optional[Artifact]:
        ...
        if artifact_file_path.exists():
            try:
                ...
                logger.error(
                    f"Error loading artifact {artifact_id} from {artifact_file_path}: {e}"
                )
        else:
            logger.warning(
                f"Artifact file {artifact_file_path} not found for ID {artifact_id} in index."
            )
    ...
    def update(self, artifact_id: str, **updates: Any) -> Optional[Artifact]:
        ...
        if changed:
            ...
            # 파일 업데이트
            artifact_filename = self.index[artifact_id].get(
                "file", f"{artifact.id}.json"
            )
            artifact_file_path = self.storage_path / artifact_filename
    ...
    def delete(self, artifact_id: str) -> bool:
        ...
        if artifact_id in self.index:
            ...
            if artifact_file_path.exists():
                try:
                    ...
                    logger.error(
                        f"Error deleting artifact file {artifact_file_path}: {e}"
                    )
                    return False
        ...
        logger.warning(
            f"Attempted to delete non-existent artifact ID in index: {artifact_id}"
        )
        return False
    ...
    def search(
        ...
        for artifact_id, info in self.index.items():
            ...
            # 태그 필터
            if tags and not any(
                tag_filter in info.get("tags", []) for tag_filter in tags
            ):
                match = False
                continue
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        most_common_tags = sorted(tag_counts.items(), key=lambda x: x[1], reverse=True)[
            :10
        ]
