...
class ChatPage:
    ...
    def _render_chat_messages(self, session: ChatSession):
        ...
        for i, msg_data in enumerate(session.messages):
            ...
            with st.chat_message(msg_data["role"]):
                ...
                # 편집 모드
                elif (st.session_state.get("editing_message_key") == msg_key and 
                      msg_data["role"] == "user"):
                ...
                # 일반 메시지 표시
                    ...
                    self._render_message_actions(session, i, msg_key, msg_data, is_last_message)
    ...
    def _prepare_user_content(self, text_input: str) -> Union[str, List[Dict[str, Any]]]:
        """사용자 콘텐츠 준비 (텍스트 + 이미지)"""
    ...
    def _get_current_model_config(self):
        ...
        provider = (st.session_state.get("selected_provider") or 
                    self.model_manager.settings.get("ui.selected_provider"))
        ...
        with st.chat_message("assistant"):
            ...
            try:
                ...
                if not model_config:
                    ...
                    # 비전 모델 확인
                    if (st.session_state.get("chat_uploaded_image_bytes") and 
                        not model_config.supports_vision):
    ...
    def _prepare_api_messages(self, messages: List[Dict[str, Any]], max_history: int = 10) -> List[Dict[str, Any]]:
        """API 호출용 메시지 준비"""
    ...
    def _generate_streaming_response(self, messages, placeholder) -> tuple[str, Optional[TokenUsage]]:
        """스트리밍 응답 생성"""
        full_response = ""
        final_usage = None
    ...
    def _generate_sync_response(self, messages, placeholder) -> tuple[str, Optional[TokenUsage]]:
        """동기 응답 생성"""
    ...
    def _display_usage_info(self, usage: TokenUsage):
        ...
        with st.expander("📊 토큰 사용량 (이번 응답)", expanded=False):
            cost_display = f"${usage.cost_usd:.5f}" if usage.cost_usd > 0.000001 else "$0.00"
    ...
    def _auto_generate_title(self, session: ChatSession, user_prompt: str, ai_response: str):
        """자동 제목 생성"""
...
"""
    ...
    def _retry_last_response(self, session: ChatSession):
            ...
            try:
                ...
                if usage:
                    logger.info(f"Retry successful - Tokens: {usage.total_tokens}, Cost: ${usage.cost_usd:.5f}")
            except Exception as e:
