...
def render_spotify_page(spotify_manager: SpotifyManager, ui_components: EnhancedUI):
    ...
    if not spotify_manager.is_configured():
        render_spotify_setup(spotify_manager)
...
def render_spotify_setup(spotify_manager: SpotifyManager):
    ...
    # 현재 저장된 설정 불러오기
    settings_manager = spotify_manager.settings_manager
        ...
        if submitted:
            if client_id and client_secret and redirect_uri:
                try:
                    ...
                    success = spotify_manager.save_spotify_settings(
                        client_id=client_id,
    ...
    with st.expander("🤔 설정 방법이 궁금하다면", expanded=False):
        st.markdown(
            ...
            2. **"Create app"** 클릭
            3. 앱 정보 입력:
               - **App name**: 원하는 이름 (예: "My LLMOS App")
