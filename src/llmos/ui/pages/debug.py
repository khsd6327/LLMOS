...
class DebugPage:
    ...
    def _render_settings_section(self):
        ...
        for provider, status in validation_result["provider_status"].items():
            with st.expander(f"{provider.upper()}", expanded=False):
                ...
                with col1:
                    ...
                    st.write(f"인터페이스: {'✅' if status['interface_initialized'] else '❌'}")
                ...
                with col2:
                    ...
                    if status['interface_initialized']:
                        interface = self.model_manager.get_interface(
                            next(p for p in self.model_manager.get_available_providers() if p.value == provider)
                        )
                        if interface:
        ...
        if st.button("전체 설정 보기 (JSON)", key="view_full_settings_btn"):
            st.session_state.show_full_settings = not st.session_state.get("show_full_settings", False)
    ...
    def _render_sessions_section(self):
        ...
        if current_session:
            ...
            with st.expander(f"세션: {current_session.title}", expanded=False):
                ...
                if current_session.messages:
                    user_msgs = sum(1 for m in current_session.messages if m["role"] == "user")
                    assistant_msgs = sum(1 for m in current_session.messages if m["role"] == "assistant")
        ...
        if all_sessions:
            ...
            for session in all_sessions[:20]:  # 최대 20개만 표시
                session_data.append({
                    "ID": session.id[:8] + "...",
                    "제목": session.title[:30] + ("..." if len(session.title) > 30 else ""),
    ...
    def _render_test_section(self):
        ...
        if st.button("API 연결 테스트", key="test_api_connection_btn"):
            provider_enum = next(p for p in available_providers if p.name == selected_provider)
        ...
        if st.button("스트리밍 테스트", key="test_streaming_btn"):
            try:
                test_messages = [{"role": "user", "content": "1부터 10까지 세어주세요."}]
                
                st.info("스트리밍 테스트 시작...")  # ← placeholder.info가 아니라 st.info
                ...
                with st.empty() as placeholder:
                    ...
                    # 현재 선택된 테스트 제공업체 사용
                    selected_test_provider = st.session_state.get("test_provider_selector", available_providers[0].name)
                    ...
                    for chunk, usage in self.model_manager.stream_generate(
                        ...
                        try:
                            ...
                            placeholder.text(f"스트리밍 응답 (청크 {chunk_count}개): {accumulated_text}")
                    ...
                    if accumulated_text.strip():
                        success_msg = f"✅ 스트리밍 테스트 완료! 총 {chunk_count}개 청크 수신"
                        if final_usage:
                            success_msg += f" | 토큰: {final_usage.total_tokens}"
                        st.success(success_msg)  # ← placeholder.success가 아니라 st.success
                    else:
                        st.warning("⚠️ 스트리밍 테스트 완료되었지만 응답이 비어있습니다.")
        ...
        if st.button("Streamlit 세션 상태 보기", key="view_session_state_btn"):
            st.session_state.show_session_state = not st.session_state.get("show_session_state", False)
            ...
            with st.expander("Streamlit 세션 상태", expanded=True):
                ...
                for key, value in st.session_state.items():
                    if isinstance(value, (str, int, float, bool, list, dict)) and len(str(value)) < 1000:
                        ...
                        safe_state[key] = f"<{type(value).__name__}> (크기: {sys.getsizeof(value)} bytes)"
