...
class SettingsPage:
    ...
    def __init__(self, settings_manager: SettingsManager, model_manager: EnhancedModelManager, ui: EnhancedUI):
    ...
    def _render_api_keys_section(self):
        ...
        for provider_enum in ModelProvider:
            ...
            with st.expander(f"{provider_name} API 키", expanded=False):
                ...
                # 테스트 버튼
                if has_key and col2.button(f"연결 테스트", key=f"test_apikey_{provider_val}"):
        ...
        try:
            ...
            if interface:
                ...
                with st.spinner(f"{provider.name} API 연결 테스트 중..."):
                    response, _ = interface.generate(test_messages, "test-model", max_tokens=10)
    ...
    def _render_default_settings_section(self):
        ...
        with col2:
            ...
            new_max_tokens = st.number_input(
                "최대 토큰 (기본값)",
                100, 100000, default_max_tokens, 100,
        ...
        with col1:
            if st.button("설정 초기화", key="reset_settings_btn"):
                if st.session_state.get("confirm_reset"):
                    ...
                    st.warning("정말로 설정을 초기화하시겠습니까? 다시 한 번 클릭하세요.")
