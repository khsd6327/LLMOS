...
class ArtifactsPage:
    ...
    def _render_search_view_section(self):
        ...
        with col2:
            ...
            selected_type = st.selectbox(
                "타입 필터", type_options, key="artifact_type_filter"
            )

        with col3:
            # 정렬 옵션
            sort_options = ["최신순", "오래된순", "제목순"]
            sort_order = st.selectbox("정렬", sort_options, key="artifact_sort_order")
        ...
        if all_tags:
            selected_tags = st.multiselect(
                "태그 필터", sorted(all_tags), key="artifact_tag_filter"
            )
        else:
            selected_tags = []
    ...
    def _render_artifact_card(self, artifact: Artifact):
        ...
        with st.expander(
            ...
            with col1:
                ...
                st.caption(
                    f"**생성일:** {artifact.created_at.strftime('%Y-%m-%d %H:%M')}"
                )
            ...
            # 내용 표시
            if artifact.type == ArtifactType.IMAGE and isinstance(
                ...
                self.ui.render_copy_button(
                    artifact.content, f"artifact_{artifact.id}", "내용 복사"
                )
    ...
    def _render_create_artifact_section(self):
        ...
        with st.form("create_artifact_form"):
            ...
            artifact_type = st.selectbox(
                "타입", [t.value for t in ArtifactType], key="new_artifact_type"
            )

            # 내용 입력
            if artifact_type == "image":
                uploaded_file = st.file_uploader(
                    "이미지 파일", type=["png", "jpg", "jpeg", "gif", "bmp", "webp"]
                )
                content = uploaded_file.getvalue() if uploaded_file else None
            else:
                content = st.text_area(
                    "내용", height=200, placeholder="아티팩트 내용을 입력하세요"
                )
            ...
            with st.expander("메타데이터 (선택사항)", expanded=False):
                metadata_json = st.text_area(
                    "JSON 형식의 메타데이터", placeholder='{"key": "value"}', height=100
                )
    ...
    def _render_statistics_section(self):
        ...
        if stats["by_type"]:
            ...
            type_df = pd.DataFrame(
                list(stats["by_type"].items()), columns=["타입", "개수"]
            )
    ...
    def _render_management_section(self):
        ...
        with col3:
            ...
            uploaded_artifacts = st.file_uploader(
                "아티팩트 가져오기", type=["json"], key="import_artifacts_uploader"
            )
    ...
    def _bulk_delete_artifacts(self, delete_type: str, delete_tags: str):
        ...
        try:
            # 삭제 조건에 맞는 아티팩트 검색
            search_type = (
                None if delete_type == "선택 안함" else ArtifactType(delete_type)
            )
            search_tags = (
                [tag.strip() for tag in delete_tags.split(",") if tag.strip()]
                if delete_tags
                else None
            )

            artifacts_to_delete = self.artifact_manager.search(
                type_val=search_type, tags=search_tags, limit=1000
            )
            ...
            if st.session_state.get("confirm_bulk_delete"):
                ...
                st.warning(
                    f"{len(artifacts_to_delete)}개의 아티팩트가 삭제됩니다. 계속하시겠습니까?"
                )
