    ...
    def _prepare_anthropic_args(
        ...
        for msg in messages:
            if msg["role"] == "system":
                ...
                system_prompt_text = (
                    ...
                    (content[0]["text"] if isinstance(content, list) and content and "text" in content[0] else "")
                )
            else:
        ...
        for msg_item in processed_messages:
            ...
            if merged_messages and merged_messages[-1]["role"] == current_role:
                if isinstance(current_content, str) and isinstance(merged_messages[-1]["content"], str):
                    merged_messages[-1]["content"] += f"\n{current_content}"
                    continue
                elif isinstance(current_content, list) and isinstance(merged_messages[-1]["content"], list):
                    merged_messages[-1]["content"].extend(current_content)
                    continue
                else:
                    logger.warning(f"Consecutive messages with role '{current_role}' and complex content for Anthropic. API might reject.")
            ...
            if isinstance(current_content, list):
                ...
                merged_messages.append({"role": current_role, "content": converted_content})
            elif isinstance(current_content, str):
                merged_messages.append({"role": current_role, "content": current_content})
            else:
                logger.warning(f"Unsupported message content type for Anthropic: {type(current_content)}")
            ...
            with self.client.messages.stream(
                ...
                **api_args
                ...
                for event in stream_obj:
                    if (event and   # ← 이 부분들이 새로 추가된 안전장치!
