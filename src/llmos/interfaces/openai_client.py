...
class OpenAIInterface(LLMInterface):
    ...
    def _get_model_params(self, model_id: str, **kwargs: Any) -> Dict[str, Any]:
        ...
        # o-series 모델 감지
        is_o_series = (
            model_id.startswith("o4-mini") or 
            model_id.startswith("o3-") or 
            model_id.startswith("o1-") or
            "o4-mini" in model_id or
            "o3-mini" in model_id or
            "o1-preview" in model_id or
            "o1-mini" in model_id
        )
        ...
        if 'max_tokens' in kwargs:
            if is_o_series:
                ...
                logger.info(f"o-series model detected ({model_id}): using max_completion_tokens={kwargs['max_tokens']}")
            else:
