        ...
        for msg_data_item in messages:
            if msg_data_item["role"] == "system":
                ...
                system_instruction_text = (
                    ...
                    (content[0]["text"] if isinstance(content, list) and content and "text" in content[0] else None)
                )
            else:
        ...
        for msg_item_to_process in temp_msgs_for_processing:
            ...
            if isinstance(content_data, str):
                ...
                for item_content_part in content_data:
                    ...
                    elif item_content_part["type"] == "image_url" and "image_url" in item_content_part:
                        # 텍스트 버퍼가 있으면 먼저 추가
            ...
            if parts_for_gemini:
                ...
                if gemini_contents and gemini_contents[-1]['role'] == role:
                    logger.warning(f"Consecutive '{role}' messages for Gemini. Merging logic might be needed or API may reject.")
                    if (isinstance(parts_for_gemini[0], str) and 
                        isinstance(gemini_contents[-1]['parts'][0], str) and
                        len(gemini_contents[-1]['parts']) == 1 and 
                        len(parts_for_gemini) == 1):
        ...
        **kwargs: Any
        ...
        try:
            ...
            if not candidate.content or not candidate.content.parts:
                logger.warning("Google API 응답이 정상 종료되었으나 내용(content)이 없습니다.")
                return "", None
            ...
            logger.error(f"Google API 'generate' 함수에서 심각한 오류 발생: {e}", exc_info=True)
            return "", None
    def stream(
        self,
        messages: List[Dict[str, Any]],
            ...
            for chunk in stream_response:
                try:
                    ...
                    if chunk and hasattr(chunk, 'text'):
                        ...
                        if chunk_text and chunk_text.strip():  # None 체크와 빈 문자열 체크
                            yield chunk_text
                    ...
                    logger.warning(f"Google streaming chunk has no text attribute: {ae}")
                    continue
